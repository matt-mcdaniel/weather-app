...
class App extends Component {
    ...
    fetchForecast = async cityId => {
        ...
        const response = await fetch(
            `/api/forecast?location=${cityData.city.replace(/\s/g, '+')},${
                cityData.region
            }`
        );
        const data = await response.json();
    ...
    render() {
        ...
        return (
            <div style={{padding: '45px'}}>
                ...
                {hasForecastData && (
                    <div>
                        <div style={{marginBottom: '10px'}}>Today's Weather</div>
                ...
                {!hasForecastData && (
                    <div style={{color: 'rosybrown'}}>No Forecast Data Available</div>
                )}
            </div>
        );
    }
